...
import com.easy.micro.auth.domain.Oauth2TokenDto;
 ...
 * 自定义Oauth2获取令牌接口
 *
 * 整个auth controller 只提供2个api
 * 1. 根据userName/psw获取token，调用spring-security-OAuth2.postAccessToken()
 * 2. 返回RSA publicKey，token是经过privateKey加密，client通过jks url获取publicKey只能用来验证真实性和完整性，
 *    无法直接获取payLoad里面的user，必须依赖authorization-server decode
 * 3. 区分Authorization server和 Resource server，后端提供api的是resource serve
 */
@RestController
...
public class AuthController {
    ...
    public CommonResult<Oauth2TokenDto> postAccessToken(HttpServletRequest request,
        ...
        // 通过spring-security-oauth2 jar包中的postAccessToken方法，换取token
